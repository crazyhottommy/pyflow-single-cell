...
rule scatac_allpeakcall:
	...
	shell:
		"macs2 callpeak -g hs --outdir Result/Analysis/{wildcards.sample} -n {params.name} -B -q 0.05 --nomodel --extsize=50 --SPMR -t {input.bam}"
...
if config["shortpeaks"]:
	...
	rule scatac_shortpeakcall:
		...
		shell:
			"macs2 callpeak -g hs --outdir Result/Analysis/{wildcards.sample} -n {params.name} -B -q 0.05 --nomodel --extsize=50 --SPMR -t {input.shortbam}"
